...
fun Route.deepchain() {
  ...
  post("/api/ai/status") {
    val status = call.receive<Any>()
    logger.debug("/api/ai/status from deepchain : " + status.toString())
    call.respond(HttpStatusCode.OK)
  }

  post("/api/ai/results") {
    val results = call.receive<Any>()
    logger.debug("/api/ai/results : requestBody : ${Env.gson.toJson(results)}")
    val body: Body = results as Body
    val strBody = Env.gson.toJson(body)
